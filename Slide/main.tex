...
% --- Slide 1: Hình ảnh ---
\begin{frame}{Kiến trúc TransUNet}
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{images/transunet_architecture.png}
\label{fig:kien-truc-transunet}
\end{figure}
\end{frame}
...
\begin{frame}{Tổng quan về phương pháp}
  \begin{itemize}
    \item TransUNet là một kiến trúc \textbf{lai ghép (Hybrid)}: Kết hợp sức mạnh của CNN và Transformer.
    \item \textbf{CNN (ResNet):} Đóng vai trò là bước xử lý đầu tiên để trích xuất các đặc trưng hình ảnh cơ bản (cạnh, góc, vân bề mặt).
    \item \textbf{Transformer:} Nhận đầu vào là các đặc trưng từ CNN để tiếp tục xử lý ở mức độ cao hơn.
\end{itemize}
\end{frame}

% --- Slide 3: Vai trò của Encoder ---
\begin{frame}{Encoder Transformer}
  \begin{itemize}
    \item \textbf{Vấn đề của CNN thuần túy:} Chỉ nhìn được một vùng nhỏ xung quanh (vùng tiếp nhận hạn chế), khó thấy được mối liên hệ giữa các vùng xa nhau.
    \item \textbf{Giải pháp của TransUNet:} Sử dụng Encoder là Transformer.
    \item \textbf{Tác dụng:} Giúp mô hình "nhìn" được toàn bộ bức ảnh cùng lúc, nắm bắt được \textbf{ngữ cảnh toàn cục (global context)} để phân biệt tốt hơn giữa khối u và mô lành.
\end{itemize}
\end{frame}

% --- Slide 4: Decoder ---
\begin{frame}{Decoder và Skip Connection}
  \begin{itemize}
    \item Decoder có nhiệm vụ khôi phục lại kích thước ảnh gốc để tạo ra bản đồ phân đoạn.
    \item \textbf{Cải tiến quan trọng:} Sử dụng \textbf{Skip Connection} để kết hợp:
    \begin{itemize}
        \item Thông tin ngữ cảnh từ Transformer (giúp định vị u đúng chỗ).
        \item Thông tin chi tiết từ CNN (giúp vẽ biên u sắc nét).
    \end{itemize}
    \item Kết quả là vùng phân đoạn vừa chính xác về vị trí, vừa chi tiết về hình dáng.
\end{itemize}
\end{frame}
...
% --- Slide 1: (Bảng số liệu) ---
\begin{frame}{Kết quả thực nghiệm: TransUNet}
\begin{table}[H]
  \centering
  \caption{Kết quả định lượng trên tập kiểm thử}
  \begin{tabular}{
      >{\raggedright\arraybackslash}p{2cm} 
      >{\centering\arraybackslash}p{2cm}
      >{\centering\arraybackslash}p{2cm}
    ...
    \hline
     & \textbf{Dice} & \textbf{IoU} & \textbf{ASD} & \textbf{HD95} \\
    \hline
    \textbf{WT} & 0.8815 & 0.7950 & 2.5640 & 7.8920 \\
    \textbf{TC} & 0.8105 & 0.7150 & 3.1020 & 9.0540 \\
    \textbf{ET} & 0.7240 & 0.6050 & 2.8560 & 8.1250 \\
    \hline
  \end{tabular}
\end{table}
\end{frame}

% --- Slide 2: (Hình ảnh thực tế) ---
\begin{frame}{Kết quả phân đoạn trực quan}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{images/transunet_qualitative_result.png}
    \caption{So sánh nhãn thực tế (Hàng trên) và kết quả dự đoán của TransUNet (Hàng dưới).}
    \label{fig:transunet-visual-res}
\end{figure}
...
\begin{center}
    \footnotesize 
    \textcolor{red}{$\blacksquare$ WT} \quad 
    \textcolor{green}{$\blacksquare$ TC} \quad 
    \textcolor{blue}{$\blacksquare$ ET}
\end{center}
\end{frame}
...
TransUNet   & 0.8815 & 0.7950 & 2.5640 & 7.8920 \\
...
TransUNet   & 0.8105 & 0.7150 & 3.1020 & 9.0540 \\
...
UNETR       & 0.6869 & 0.5788 & 4.2527 & 12.4837 \\
TransUNet   & 0.7240 & 0.6050 & 2.8560 & 8.1250 \\
